...
def main():
    ...
    input = torch.randint(
        0, config.vocab_size, size=(config.batch_size, config.seq_length)
    )
    output = torch.concatenate((input[:, 1:], torch.randint(0, config.vocab_size, size=(config.batch_size,1))), dim=1)
