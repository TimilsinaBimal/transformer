...
class DotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V):
        ...
        # mask here
        if self.mask:
            look_ahead_mask = torch.tril(torch.ones(x.size()), diagonal=0)
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.inp_embeddings = Embeddings(vocab_size=vocab_size, seq_length=seq_length)
        self.out_embeddings = Embeddings(vocab_size=vocab_size, seq_length=seq_length)
        self.encoder = Encoder(n_x=n_x, num_heads=num_heads, d_model=d_model)
        self.decoder = Decoder(n_x=n_x, num_heads=num_heads, d_model=d_model)
...
def main():
    ...
    output = torch.randint(0, vocab_size, size=(batch_size, seq_length))
